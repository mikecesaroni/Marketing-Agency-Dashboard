...
export const ROUTES = [
  ...
  },
  {
    ...
    nav: 'Tasks',
    purpose:
      'The team\u2019s own to-do board, ClickUp underneath: a task has a status, priority, assignees, dates, tags, subtasks, a checklist and comments. Four views instead of a folder tree.',
    contains: [
    ...
    actions: [
      'Type a task and press Enter. Click a row or card to open it; every field saves on change.',
  ...
  },
  {
    ...
    actions: [
      ...
      'MRR counts a client only when they have a scheduled monthly payment, and never counts archived or paused clients. A client with a monthly fee typed in but no schedule contributes nothing — that is the usual explanation for MRR looking low.',
      'A paused client\'s scheduled months from the pause day on show "on pause": not owed, not overdue, not upcoming, not outstanding. Months due before the pause are still owed, and a setup fee still owed is still listed.',
    ],
  },
  {
    path: '/reports',
...
export const LOOKUPS = [
  ...
  },
  {
    ...
    how: 'It opens with every client in the figure and their monthly fee, biggest first, adding up to the total — plus a "Not counted" list naming everyone left out and why (archived, one of ours rather than a client, on pause, or no monthly schedule yet). That second list is the half that settles "but I thought we had more clients than that".',
  },
  {
    need: 'A Stripe payment that does not appear anywhere',
    where: 'Payments → Stripe → the unmatched queue',
    how: 'Every payment on the connected Stripe account lands either against a client or in that queue — including subscription checkouts and failed charges, which used to be dropped or reported and then written nowhere. Nothing guesses whether a payment is for this business: assign it to a client, or dismiss it. A dismissal covers THAT ONE PAYMENT and nothing more -- it builds no rule and remembers no customer, so if the same person pays again next month it shows up again and gets dismissed again. That is deliberate. Dismissing used to add the customer and email to an ignore list the webhook checked before parking anything, and it silently swallowed a real $998 client invoice from an email dismissed the day before over an unrelated charge. Every payment, every time, is worth more than saving a click. A parked payment is money that exists, so an empty queue is the only state that means nothing is outstanding.',
  },
  {
    need: 'Why MRR looks wrong',
    where: 'Payments → the Stripe reconciliation panel',
    how: 'It names each client where Stripe collected a different amount to the CRM\'s figure. Stripe is right; correct the CRM. Also check the client is not archived or on pause and does have a scheduled monthly payment.',
  },
  {
  ...
  },
  {
    ...
    how: 'Tick a video and press "Write the copy" to have Claude write the primary text, headline and description from that client\'s own onboarding answers — their offer, guarantee, price range and the reason people choose them. It will not invent a rating, a review count or a discount that is not on file; where the intake is thin it says so in the note and writes only what it can stand behind. "Another angle" swaps in the next of three versions it already wrote before spending another request. Everything it writes lands in editable boxes. Upload .mp4 or .mov there and it goes into the client\'s files AND straight to their Meta ad account, because Meta transcodes asynchronously and an ad cannot use a video until that finishes — usually well under a minute. The pill on each video says where it has got to. A video is only tickable once it reads "Ready to publish", and ticked videos publish as ads in the SAME ad set as the image creatives, so a video and a static are testable against each other on one budget. A Meta video belongs to one ad account, so the same clip used for two clients is uploaded to each of them separately.',
  },
  {
...
export const GOTCHAS = [
  ...
  {
    title: 'There is no button on the ad image',
    body: 'The creative carries no call-to-action button, deliberately. Meta renders the real one directly under the image and it is chosen on the Publish tab, so a painted one inside the frame was a decoy nobody could click — and it could contradict the real button, which said "Learn More" while the image said "Book Today!". If copy needs to push toward the button it points down and out of the image, and it belongs in the primary text.',
  },
  {
