...
/**
 * How a client pays for GHL.
 *
 *   separate  two Stripe subscriptions and two invoices a month, so two
 *             payment schedules here: the retainer and a $399 GHL row.
 *   bundled   one combined subscription that already includes GHL. One
 *             invoice, one schedule, and the GHL share is bookkeeping only.
 *
 * The distinction is not cosmetic. Giving a bundled client a separate $399
 * schedule would leave one of the two schedules permanently unpaid, because
 * only one invoice ever arrives to settle them.
 */
export const GHL_BILLING = {
  separate: {
    key: 'separate',
    label: 'Billed separately',
    short: 'separate charge',
    help: 'A second Stripe subscription. Two invoices a month, tracked as two payments.',
  },
  bundled: {
    key: 'bundled',
    label: 'Included in the monthly fee',
    short: 'in the monthly fee',
    help: 'One combined subscription. A single invoice covers the retainer and GHL together.',
  },
}

// Null for anyone not on the plan, so callers cannot accidentally read a
// billing mode off a client who does not have GHL at all. The column carries a
// default, which is meaningless until ghl_plan is turned on.
export function ghlBilling(client) {
  if (!client?.ghl_plan) return null
  return GHL_BILLING[client.ghl_billing] || GHL_BILLING.bundled
}

/**
 * The monthly revenue attributable to GHL for one client.
 *
 * The same number either way, but it means two different things: money
 * invoiced on its own for a separate client, and a share of a larger invoice
 * for a bundled one. That is exactly why it must not be added to the retainer
 * for a bundled client — the combined fee already contains it.
 */
export function ghlMonthlyPortion(client) {
  if (!client?.ghl_plan) return 0
  return Number(client.ghl_monthly_fee) || 0
}

// What the client is billed in total each month, however it is split up.
export function totalMonthly(client) {
  const retainer = Number(client?.monthly_fee) || 0
  return ghlBilling(client)?.key === 'separate' ? retainer + ghlMonthlyPortion(client) : retainer
}
